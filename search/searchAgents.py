...
class SearchAgent(Agent):
    ...
    def registerInitialState(self, state):
        ...
        print('Path found with total cost of %d in %.1f milliseconds' % (totalCost, int(round(time.time() * 1000)) - starttime))
...
def foodHeuristic(state, problem):
    ...
    for x2, row in enumerate(foodGrid):
        for y2, cell in enumerate(row):
            if foodGrid[x2][y2]:
                cost.append(mazeDistance(position, (x2, y2), problem.startingGameState))
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        """
            Since we have to search path to closest dot, we can use Uniform Cost search to do that.
        """
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        for goal in self.food.asList():
            manhattan_distance_list.append((util.manhattanDistance(state, goal), goal))
        distance, goal = min(manhattan_distance_list)
        if state == goal:
            return True
        else:
            return False
